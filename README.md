...
*Building-level damage class per epoch (E2a, E2b, E3), drift-corrected relative
coherence loss against the pre-conflict reference (E1). The affected share
above each panel is the same drift-corrected grid figure reported throughout
this document.*
...
confirm the SAR extent. The figure reports the correlation rather than a bare
agreement score, which would treat the spectrally blind optical signal as
ground truth and read as false agreement because both classes are dominated by
intact cells. The full confusion matrix, with precision and recall, is in the
linked report.*
...
The code in this repository is MIT-licensed (below). The third-party datasets it
draws on keep their own licences; the MIT licence does not relicense them. Every
source used here permits reuse and redistribution with attribution.

- **Sentinel-1A SLC** (Copernicus / ESA, retrieved from the
  [Copernicus Data Space Ecosystem](https://dataspace.copernicus.eu/)): free
  Copernicus data. Contains modified Copernicus Sentinel data (2023). Output CRS
  EPSG:32634 (UTM zone 34N).
- **Sentinel-2 L2A** (Copernicus / ESA, streamed as cloud-optimized GeoTIFFs from
  the [Element84 Earth Search STAC](https://earth-search.aws.element84.com/) for
  the dNBR optical cross-validation in `validate_optical.py`): free Copernicus
  data. Contains modified Copernicus Sentinel data (2023).
- **Building footprints** ([Humanitarian OpenStreetMap Team (HOT)](https://www.hotosm.org/)
  and OpenStreetMap contributors): [Open Database License (ODbL)](https://opendatacommons.org/licenses/odbl/).
  The map figures carry the © OpenStreetMap contributors credit.
- **SRTM 3Sec DEM** (NASA / USGS, downloaded automatically by SNAP for terrain
  correction and geocoding): public domain; credited as a courtesy, no
  attribution required.
- **Country boundaries and rivers** ([Natural Earth](https://www.naturalearthdata.com/)
  admin-0 countries and rivers/lake centerlines, 50 m; study-area locator): public
  domain; credited as a courtesy, no attribution required.

To reproduce the analysis, each dataset must be obtained from its own provider
under these terms (see [docs/DATA.md](docs/DATA.md)). This repository does not
redistribute the raw Sentinel scenes or the OpenStreetMap download; it ships only
the small derived map figures the analysis produces.
