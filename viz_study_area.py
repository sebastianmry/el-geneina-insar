...
def draw_main(ax, countries: gpd.GeoDataFrame) -> None:
    ...
    ax.annotate("El Geneina", (cx, cy), textcoords="offset points",
                xytext=(0, 6), ha="center", va="bottom", color=config.COLOR_FG,
...
def _draw_aoi_reticle(ax, cx, cy) -> None:
    """Draw a solid box around the analysis AOI as a callout that frames the city.

    The true AOI is only a few kilometres across, invisible at this scale, so the
    box is drawn at a fixed on-screen size to mark where the analysis sits.
    """
    ...
    half = 115000.0          # box half-size in metres (tight frame on the city)
    x0, x1 = ax_cx - half, ax_cx + half
    y0, y1 = ax_cy - half, ax_cy + half
    ax.plot([x0, x1, x1, x0, x0], [y0, y0, y1, y1, y0],
            color="#f5c518", linewidth=1.3, alpha=0.85, zorder=6,
...
def _draw_west_darfur(ax) -> None:
    ...
    label_x, label_y = _merc_xy(25.0, 15.25)    # open ground just east of Darfur
